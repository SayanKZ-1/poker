...
export class GameService {
 ...
 async login(name,oldToken) {
  ...
  await this.db.query('insert into private.sessions values($1,$2,$3)',[await hash(token),this.db.json(user),expires]);
 ...
 async create(user,data) {
  ...
  await this.db.transaction(async tx=>{
   ...
   await tx.query('insert into private.rooms(id,owner_id,state,invite_token,short_code) values($1,$2,$3,$4,$5)',[r.id,user.id,tx.json({...r,table:t.serialize()}),r.inviteToken,r.code]);await this.save(tx,r);
  });return {roomId:r.id};
 }
 ...
 async command(user,d){
  ...
  return this.locked(d.roomId,async(tx,r)=>{
   ...
   await tx.query('insert into private.commands(room_id,actor_id,request_id,fingerprint,result) values($1,$2,$3,$4,$5)',[r.id,user.id,d.requestId,fingerprint,tx.json(result)]);return result;
